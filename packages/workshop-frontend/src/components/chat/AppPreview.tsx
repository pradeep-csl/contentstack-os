  ...
  return (
    <div className="flex flex-col h-full bg-kumo-base">
      ...
      <div className="flex-1 overflow-auto p-6">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-ui-md font-semibold text-kumo-default">{row.channel}</span>
        ...
        {/* Quiet channels */}
        <div className="mt-6">
          <div className="text-ui-xs font-semibold text-kumo-subtle uppercase tracking-wider mb-3">
            No new activity
          </div>
          <div className="flex flex-wrap gap-2">
            {sampleDataRows.filter(r => !r.unread).map((row) => (
              <div key={row.id} className="px-3 py-1.5 rounded-md bg-kumo-tint">
                <span className="font-mono text-ui-xs text-kumo-subtle">{row.channel}</span>
              </div>
            ))}
          </div>
        </div>
