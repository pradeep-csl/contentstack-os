...
export function GadgetPresence({
  ...
  return (
    <Popover>
            ...
            <span ref={stackRef} className="relative flex -space-x-2">
              ...
              {overflow > 0 && (
                <span data-flip-id="__overflow" className="inline-flex cursor-pointer">
                  <span
                    className="presence-pop-in grid h-[26px] w-[26px] place-items-center rounded-full bg-kumo-tint text-ui-2xs text-kumo-strong ring-2 ring-kumo-base"
                  >
                    +{overflow}
                  </span>
                </span>
      ...
      >
        <Popover.Title className="shrink-0 px-2.5 pt-1.5 pb-1 text-ui-2xs text-kumo-subtle">
          {label}
        </Popover.Title>
          ...
          {display.map((p) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-ui-xs font-medium text-kumo-default">
                  {p.user.name}
                </span>
