...
const buttonBaseClassName =
  'inline-flex cursor-pointer items-center justify-center rounded-lg text-ui-sm font-medium transition-[background-color,color,opacity,transform] duration-150 ease-out active:scale-[0.98] disabled:cursor-not-allowed disabled:active:scale-100'
...
export function WorkshopInput({ className = '', ...props }: WorkshopInputProps) {
  return (
    <Input
      {...props}
      className={`!h-9 rounded-lg border border-kumo-line bg-kumo-base px-3 text-ui-sm font-normal text-kumo-default placeholder:text-kumo-placeholder shadow-none focus:border-kumo-ring focus:outline-none focus:ring-1 focus:ring-kumo-ring/15 ${className}`}
    />
  )
}
...
export function WorkshopInputArea({ className = '', ...props }: WorkshopInputAreaProps) {
  return (
    <InputArea
      {...props}
      className={`rounded-lg border border-kumo-line bg-kumo-base px-3 py-2 text-ui-sm font-normal text-kumo-default placeholder:text-kumo-placeholder shadow-none focus:border-kumo-ring focus:outline-none focus:ring-1 focus:ring-kumo-ring/15 ${className}`}
    />
  )
}
