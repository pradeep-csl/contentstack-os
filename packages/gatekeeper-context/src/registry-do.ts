...
export class LibraryRegistryDurableObject extends DurableObject<Cloudflare.Env> {
  ...
  // snapshot's actual consumers (`getEnabledCollections`, `loadEnabledContextCollections`) never read
  // `documentCount`, and only use `lastUpdated` for presentation recency, so we only rewrite when a
  // field a consumer reads for identity — title, description, icon, visibility — has changed.
  // Trade-off, deliberately accepted: `documentCount` and `lastUpdated` in the snapshot go stale
  // between identity edits. Membership (`addPublic`/`removePublic`) is unaffected and still writes
  // unconditionally.
