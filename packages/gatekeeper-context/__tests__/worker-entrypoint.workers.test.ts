...
function post(path: string, body: unknown = { commit: "c1", manifest: [] }) {
...
describe("worker entrypoint", () => {
  ...
  it("blocks a request over the rate limit before the collection is resolved", async () => {
    ...
    // vitest.workers.config.ts) is exhausted by the first call.
    let path = "/gatekeeper/context/ingest/test/entrypoint-ratelimit/plan";

    // Under the limit: the limiter lets the request through, so it reaches handleIngestRequest and
    // fails auth exactly like the "unauthenticated" test above — proving this call really was
    // resolved.
    let first = await post(path);
    expect(first.status).toBe(401);

    // Over the limit: if this call were still reaching handleIngestRequest, it would hit the same
    // missing-token check as `first` and also return 401. Getting 429 instead is direct evidence the
    // limiter short-circuited before the collection was ever resolved.
    let second = await post(path);
