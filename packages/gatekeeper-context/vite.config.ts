...
export default defineConfig({
  run: {
    tasks: {
      ...
      // separate commands so the node suite can replay when only the workers suite's inputs moved.
      test: vitestTask(['vitest run', 'vitest run -c vitest.workers.config.ts']),
