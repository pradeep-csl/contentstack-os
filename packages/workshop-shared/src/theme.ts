...
// The dark-mode lightness is pinned at 0.45 to guarantee white button label text stays legible:
// L <= 0.45 is the only threshold that clears WCAG AA across all hues (worst case is cyan, ~4.95:1).
// This does not affect deployments with no custom accent, because those pass "" to
// applyAccentColor(), which clears these variables so the static tokens.css default applies.
...
/**
 * Every custom property {@link applyAccentColor} sets. Exported so a test can assert the clear path
 * removes all of them, without hand-maintaining a second copy of the list that could silently drift.
 */
export const ACCENT_VAR_NAMES = Object.keys(accentVariables("#000"));
...
export function applyAccentColor(
  ...
  if (!color || !isHexColor(color)) {
    for (const name of ACCENT_VAR_NAMES) target.removeProperty(name);
    return;
  }
