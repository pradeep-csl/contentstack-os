...
export type AdminSettingsView = {
  ...
  siteName: string;
  /** Custom deployment logo, or undefined to use the default Contentstack OS mark. */
...
export interface AdminApi {
   ...
   * image, or undefined after reset. Pass null to restore the default Contentstack OS mark. The
...
/**
 * A model as offered in pickers and settings lists. Extends AiChatAuthorInfo with the gateway that
 * serves it, for the origin tag in the UI.
 *
 * IMPORTANT: only `listModels()` returns this. AiChatAuthorInfo is embedded in every chat message
 * and stays lightweight, so a gateway-tagged profile must never reach the chat log -- in particular
 * CloudflareModelGateway.resolveModel() keeps returning a bare AiChatAuthorInfo.
 */
