...
function kumoFiles(dir: string, out: string[] = []): string[] {
...
function bareInactiveIn(text: string): boolean {
...
const INACTIVE_TOKEN_CASES: [string, boolean][] = [
...
const PENDING_SIZES = new Set<string>([])
...
const BANNED_SIZE_CASES: [string, boolean][] = [
  ['text-[13px]', true],
  ['text-[11.5px]', true],
  ['text-sm', true],
  ['text-ui-xs', false],
...
const BANNED_TRACKING_CASES: [string, boolean][] = [
...
function has2xsTrackingCollision(line: string): boolean {
  return TWOXS_TOKEN.test(line) && TRACKING_UTILITY.test(line)
}

const TWOXS_TRACKING_CASES: [string, boolean][] = [
...
function has2xsWeightCollision(line: string): boolean {
  return TWOXS_TOKEN.test(line) && WEIGHT_UTILITY.test(line)
}

const TWOXS_WEIGHT_CASES: [string, boolean][] = [
...
function missingStrongRole(line: string): boolean {
  return HEADING_STEP.test(line) && !STRONG_ROLE.test(line)
}

const HEADING_STEP_CASES: [string, boolean][] = [
