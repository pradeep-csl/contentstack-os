...
/**
 * A Rate Limiting binding declaration. Unlike the other bindings its configuration is inline
 * rather than account-provisioned, so it needs no placeholder and passes through verbatim.
 */
export interface RateLimitBinding {
  /** Binding name the worker reads. */
  name: string;
  /** Namespace id, unique within the worker. */
  namespace_id: string;
  /** The window this limiter enforces. */
  simple: {
...
export interface WranglerConfig {
  ...
  /** Rate Limiting bindings (gatekeeper-context's CI ingestion endpoint). */
...
export function buildWorkerEntry(
  ...
  for (const limiter of config.ratelimits ?? []) {
    // No placeholder: the limit is inline configuration, not an account-provisioned resource.
    bindings.push({
      type: "ratelimit",
      name: limiter.name,
